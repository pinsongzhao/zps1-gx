...
def create_stamp_file(
    ...
    x_image = cat["xcentroid"] + 1
    y_image = cat["ycentroid"] + 1
...
def cut_stamps(
    ...
    for i in range(len(stamps)):
        ...
        if cut_list[i] == 1:
            ...
            # define the pixel boundaries inclusive, 0-based
            ...
            xlo1 = int(round(x0 - (ximg - xlo) * pixel_scale / ps))  # convert from original pixel scale.
            xhi1 = int(round((xhi - ximg) * pixel_scale / ps + x0))  # convert from original pixel scale.
            ylo1 = int(round(y0 - (yimg - ylo) * pixel_scale / ps))  # convert from original pixel scale.
            yhi1 = int(round((yhi - yimg) * pixel_scale / ps + y0))  # convert from original pixel scale.
            ...
            # If no overlap, skip
            if xlo2 > xhi2 or ylo2 > yhi2:
                print(f"ID {id}: skipped (no overlap)")
                continue

            stamp_nx = xhi2 - xlo2 + 1
            stamp_ny = yhi2 - ylo2 + 1
            stamp_data = data[ylo2:yhi2+1, xlo2:xhi2+1].copy()
