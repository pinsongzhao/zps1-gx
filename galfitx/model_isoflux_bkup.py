...
class GalfitModelGenerator:
    ...
    def get_flux_fraction(
        ...
        x = stamps.columns[1][stamp_idx]
        y = stamps.columns[2][stamp_idx]
        xlo = stamps.columns[5][stamp_idx]
        xhi = stamps.columns[6][stamp_idx]
        ylo = stamps.columns[7][stamp_idx]
        yhi = stamps.columns[8][stamp_idx]
...
class BackgroundAnalyzer:
    ...
    @staticmethod
    def check_overlapping_apertures(positions: np.ndarray, radius: float) -> np.ndarray:
        """
        ...
        overlap_matrix = (dist_matrix < 2.0 * radius) & (dist_matrix > 0)
    ...
    def decide_background_positions_with_distance_map(
        ...
        non_overlapping = self.check_overlapping_apertures(positions, aperture_radius)
    ...
    def _save_region_file(self, band: str, aperture_radius: float, positions: np.ndarray, output_path: str) -> None:
        ...
        with open(filename, "w") as f:
            ...
            for pos in positions:
                x, y = pos[1], pos[0]
                f.write(f"circle({x:.4f},{y:.4f},{aperture_radius:.1f})\n")
    ...
    def compute_background_noise(
        ...
        for radius in aperture_radii_px:
            ...
            bkg_positions = list(zip(text[0].str[7:].astype(float), text[1]))
